    ...
    if sort:
        for elem in tree.iter():
            if elem.attrib:  # Only sort if there are attributes
                ...
                elem.attrib.clear()
                elem.attrib.update(sorted_attrs)  # Update in-place (avoids extra dict)
...
def beautify(
    ...
    Args:
      ...
      sort (bool, optional): Flag indicating whether to sort the file based on element alphabetical order. Defaults to False.
        ...
        if parts[0].startswith(b"<?xml"):
            ...
            if declaration == "auto":
                if parts[2].find(b'="') != -1:
                    ...
                    logging.debug(f"Detected use of double quotes in {filename}")
                elif parts[2].find(b"='") != -1:
                    declaration = "single-quotes"
                    logging.debug(f"Detected use of single quotes in {filename}")
                else:
                    declaration = "double-quotes"
                    logging.warning(
            ...
            if declaration == "double-quotes":
                ...
                logging.debug(f"Double quotes enforced in declaration: {filename}")
            elif declaration == "single-quotes":
                xml = parts[0].replace(b'"', b"'") + parts[1] + parts[2]
                logging.debug(f"Single quotes enforced in declaration: {filename}")
        else:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="sort",
        help="Sort the file alphabetically",
    )
    ...
    sort: bool = str_to_bool(os.environ.get(f"{ENV_PREFIX}SORT", str(args.sort)))
    endings = os.environ.get(f"{ENV_PREFIX}LINE_ENDINGS", args.endings).lower()
    self_closing = os.environ.get(
        f"{ENV_PREFIX}SELF_CLOSING", args.self_closing
    ).lower()
    declaration = os.environ.get(f"{ENV_PREFIX}DECLARATION", args.declaration).lower()
